...
def _format_panel(artifacts: DemoArtifacts) -> str:
    ...
    in_target = 0.35 <= artifacts.saving_rate <= 0.62
    target_badge = (
        "✅ In target range (35%–62%)"
        if in_target
        else "⚠️ Out of target range"
    )
    ...
    return (
        ...
        f"{small_line}\n"
        "[Final Executable Prompt | Cloud LLM Simulation]\n"
        f"{artifacts.executable_prompt}\n"
        f"{small_line}\n"
        f"[Savings Report]\n"
...
def run_demo() -> None:
    ...
    - This addresses enterprise pain points where token cost pressure is severe
      (85/100) and latency pressure is also high (70/100).
    """

    # ~200-token style verbose request for RAG competitor analysis simulation.
    original_request = (
        "for our strategy review, compare Apple and Tesla using retrieved "
        "annual-report context, earnings commentary, and r-and-d investment "
        "notes, then evaluate innovation efficiency, long-cycle product "
        "momentum, and moat durability across both firms. i also need a "
        "practical interpretation for enterprise decision-making: where each "
        "company is likely to sustain competitive advantage, where execution "
        "risk is rising, and which signals should be monitored in upcoming "
        "quarters. please return the result in chinese, keep it compact but "
        "information-dense, and output 5 bullet points that summarize key "
        "competitive strengths so leadership can quickly compare strategic "
        "positioning and feed the findings into an internal investment and "
        "partnership memo."
    )
    ...
    try:
        ...
        # If tiktoken is unavailable, keep demo usable with a deterministic fallback.
        ...
        print(
            "⚠️ tiktoken not available; "
            "using rough word-based estimate for token counts."
        )
