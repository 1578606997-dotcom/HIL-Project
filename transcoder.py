...
class HILTranscoder:
    ...
    def decode(self, hil_cmd: str) -> str:
        ...
        if parsed.object_symbol == "@vs" and parsed.object_args:
            object_phrase = f"a compare-and-contrast analysis between {', '.join(parsed.object_args)}"
    ...
    def reverse_translate(
        ...
        """Translate natural language to HIL using rules first, then optional LLM refinement."""
        ...
        if any(k in lowered for k in ("create", "write", "generate", "生成", "写")):
            action = "!"
        elif any(k in lowered for k in ("transform", "convert", "rewrite", "改写", "转换")):
            action = ">"

        object_part = "$"
        if "top" in lowered and any(k in lowered for k in ("advantage", "优势", "竞争")):
            object_part = "@top"
        ...
        count_match = re.search(r"(?:\b|第)(\d{1,3})(?:\s*(?:points?|条|项))?", lowered)
